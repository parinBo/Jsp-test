...
var xmlToJson = (function () {
  ...
  self.parseAttributes = function (node) {
	...
	* @returns none
	*/
  self.parseChildren = function (parent, childNodes,withAttr) {
    ...
    if (childNodes.length > 0) {
      ...
      for (var i = 0; i < childNodes.length; i++) {
        ...
        if (childNodes[i].nodeType == 1) {
          self.parseNode(parent, childNodes[i],withAttr);
        }
      }
    }
  };
	...
	* @returns {Object}
	*/
  self.parseNode = function (parent, node,withAttr) {
	  withAttr = (withAttr == undefined) ?false :withAttr;
    ...
    if (node.childNodes.length == 1 && node.childNodes[0].nodeType == 3) {
      ...
      if (node.hasAttributes() && withAttr) {
    ...
    // Otherwise, there are child XMLNode elements, so process them
      ...
      self.parseChildren(obj, node.childNodes,withAttr);
    }
  ...
  // Expose the API
  return {
    parse: function (xml,withAttr) {
      if (xml && typeof xml === 'string') {
        xml = self.convertXMLStringToDoc(xml);
      }
      return (xml && self.isXML(xml)) ? self.parseNode({}, xml.firstChild,withAttr) : null;
    }
  }
})();
